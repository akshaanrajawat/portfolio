...
const Taskbar = ({ openWindows, onStartClick, onWindowClick, isStartMenuOpen }: TaskbarProps) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 h-12 taskbar-glass flex items-center justify-between px-2 z-[9000]">
      {/* Start Button */}
      <button
      ...
      <div className="flex-1 flex items-center gap-1 mx-2 overflow-x-auto">
        {openWindows.map((window) => (
          <button
            key={window.id}
            className={`h-9 px-4 rounded flex items-center gap-2 transition-all ${
              window.isMinimized
          ...
          >
            <span className="text-white text-sm truncate max-w-[150px]">{window.title}</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="h-9 px-3 flex flex-col items-end justify-center text-white text-xs border-l border-white/20 ml-2">
          <div className="font-medium">{formatTime(time)}</div>
          <div className="text-white/80">{formatDate(time)}</div>
        </div>
      </div>
    </div>
  );
